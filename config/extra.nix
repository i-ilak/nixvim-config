...
}:
{
  ...
  extraConfigLua = ''
    vim.api.nvim_create_autocmd("FileType", {
      callback = function()
        ...
        if not vim.b[buf].jinja_syntax_autocmd_loaded then
          ...
          -- trailbalzer_localark as loaded for this buffer
    ...
    function trailbalzer_local_init()
      ...
      function trailbalzer_local_save_all_sessions()
        ...
        if vim.tbl_isempty(buffers) then
          ...
          for _, buf in ipairs(buffers) do
            trailbalzer_local.save_session(buf)
          end
        end
      end

      function trailbalzer_local_save_session(buf)
      ...
      autocmd("VimLeavePre", {
        pattern = "*",
        callback = function(_)
          trailbalzer_local.save_all_sessions()
        end,
      })
    end

    -- Implement these functions directly in the key-mappings!

    -- function trailbalzer_local_new_mark()
    ...
    -- function trailbalzer_local_load_session()
    --   local tb = require("trailblazer")
